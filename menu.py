...
DB_CONFIG = {
    ...
    "password": "1234",
    "port":"3307",  # Reemplaza con la contraseña real o déjalo vacío si no tiene
    "database": "mydb"         # Nombre de la base de datos a utilizar
...
def sp_insertar(id_usuario: int, nombre: str, contraseña: str, tipo_usuario_id: int) -> int:
    """
    ...
    try:
        ...
        args = [id_usuario,nombre, contraseña, tipo_usuario_id]  # Prepara los argumentos, el último es OUT
        args = cur.callproc("sp_usuario_insertar", args)  # Llama al procedimiento almacenado
        cnx.commit()               # Confirma la transacción (hace persistente la inserción)
        nuevo_id = args[0]         # Recupera el valor OUT (el nuevo ID generado)
...
def sp_listar_activos():
    ...
    try:
        ...
        cur.callproc("sp_usuario_listar_activos")  # Ejecuta el SP sin parámetros
...
def sp_listar_todos():
    ...
    try:
        ...
        cur.callproc("sp_usuario_listar_todo")  # 3️⃣ Llama al procedimiento almacenado
...
def menu():
    ...
    while True:
        ...
        if opcion == "1":
            ...
            try:
                id_usuario = int(input("ingrese su id de usuario (ej: 1)"))
                tipo_usuario_id = int(input("id de tipo usuario (ej: 2): "))
            except ValueError:
                print("❌ id de tipo usuario inválido.")
                continue
            sp_insertar(id_usuario,nombre, contraseña, tipo_usuario_id)
        ...
        # --- Opción 4: Aplicar borrado lógico ---
            ...
            try:
                id_usu = int(input("ID a eliminar lógicamente: ").strip())
            except ValueError:
                print("❌ ID inválido.")
                continue
            sp_borrado_logico(id_usu)

        # --- Opción 5: Restaurar registro eliminado ---
        elif opcion == "5":
            try:
                id_usu = int(input("ID a restaurar: ").strip())
            except ValueError:
                print("❌ ID inválido.")
                continue
            sp_restaurar(id_usu)
